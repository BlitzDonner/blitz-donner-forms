...
( function () {
	...
	var cfg = typeof window.bdfrmsCaptchaConfig !== 'undefined' ? window.bdfrmsCaptchaConfig : null;
	...
	function loadSdk() {
		...
		scriptPromise = new Promise( function ( resolve, reject ) {
			var existing = document.querySelector( 'script[data-bdfrms-captcha-sdk="1"]' );
			if ( existing ) {
				loaderState = 'loaded';
				resolve();
				return;
			...
			s.defer = true;
			s.src = cfg.scriptUrl;
			s.setAttribute( 'data-bdfrms-captcha-sdk', '1' );
			s.onload = function () {
				loaderState = 'loaded';
				resolve();
			};
			s.onerror = function () {
				...
				loaderState = 'idle';
				reject( new Error( 'bdfrms-captcha-sdk-load-failed' ) );
			};
	...
	function pendingWidgets() {
		var nodes = document.querySelectorAll( '.bdfrms-captcha-row[data-bdfrms-captcha="1"]:not([data-bdfrms-captcha-armed="1"])' );
	...
	function arm() {
		...
		rows.forEach( function ( row ) {
			row.setAttribute( 'data-bdfrms-captcha-armed', '1' );
		} );
	...
	function wireForm( form ) {
		var row = form.querySelector( '.bdfrms-captcha-row[data-bdfrms-captcha="1"]' );
		if ( ! row ) {
			return;
		}
	...
	function init() {
		var forms = document.querySelectorAll( 'form.bdfrms-form' );
		Array.prototype.forEach.call( forms, function ( form ) {
			if ( form.querySelector( '.bdfrms-captcha-row[data-bdfrms-captcha="1"]' ) ) {
				wireForm( form );
			}
		} );
	}
