( function () {
	 ...
	 * @return {boolean}
	 */
	function bdfrmsWebKitDateTimeFallbackEnabled( form ) {
		if ( form ) {
			var attr = form.getAttribute( 'data-bdfrms-webkit-datetime-fallback' );
			if ( attr === '0' ) {
				return false;
			}
			if ( attr === '1' ) {
		...
		var cfg =
			typeof window.bdfrmsFrontendConfig !== 'undefined' ? window.bdfrmsFrontendConfig : null;
	 ...
	 * @return {boolean}
	 */
	function bdfrmsWebKitNeedsPlainDateTimeInputs( form ) {
		if ( ! bdfrmsWebKitDateTimeFallbackEnabled( form ) ) {
			return false;
		}
	 ...
	 * @return {boolean}
	 */
	function bdfrmsDateTimeHasServerDefault( el ) {
		return el.getAttribute( 'data-bdfrms-has-default' ) === '1';
	}
	 ...
	 * @return {void}
	 */
	function bdfrmsUpgradeWebKitDateTimeInputs( form ) {
		if ( ! form || ! bdfrmsWebKitNeedsPlainDateTimeInputs( form ) ) {
			return;
		}
		...
		form.querySelectorAll( sel ).forEach( function ( el ) {
			...
			neo.type = 'text';
			neo.setAttribute( 'data-bdfrms-wk-fallback', '1' );
			neo.className = ( el.className ? el.className + ' ' : '' ) + 'bdfrms-input--webkit-fallback';
			neo.name = el.name;
			neo.id = el.id;
			if ( el.required ) {
				neo.required = true;
			...
			if ( t === 'date' ) {
				neo.setAttribute( 'data-bdfrms-datetime-kind', 'date' );
				...
				if ( dmin ) {
					neo.setAttribute( 'data-bdfrms-min', dmin );
				}
				if ( dmax ) {
					neo.setAttribute( 'data-bdfrms-max', dmax );
				}
			} else if ( t === 'time' ) {
				neo.setAttribute( 'data-bdfrms-datetime-kind', 'time' );
				...
				neo.setAttribute( 'data-bdfrms-datetime-kind', 'datetime' );
			...
			var hasDefault = bdfrmsDateTimeHasServerDefault( el );
			neo.setAttribute( 'data-bdfrms-has-default', hasDefault ? '1' : '0' );
			if ( hasDefault ) {
	...
	function collectValues( form ) {
		...
		fields.forEach( function ( field ) {
			if ( field.type === 'password' || field.name === 'bdfrms_nonce' || field.name.indexOf( 'bdfrms_' ) === 0 ) {
				return;
			}
			if ( field.type === 'file' ) {
	...
	function syncRangeOutput( rangeInput ) {
		var wrap = rangeInput.closest( '.bdfrms-field-range' );
		if ( ! wrap ) {
			return;
		}
		var out = wrap.querySelector( 'output.bdfrms-range-value' );
		if ( out ) {
	...
	function initRangeValueDisplays( form ) {
		form.querySelectorAll( '.bdfrms-field-range input[type="range"]' ).forEach( function ( input ) {
	...
	function shouldClearByUrl( key ) {
		...
		var status = params.get( 'bdfrms_status' );
		var form = params.get( 'bdfrms_form' );
	 ...
	 *
	 * @param {HTMLElement} cipherEl  – .bdfrms-submit-overlay__cipher
	 */
	function bdfrmsRunCipherAnimation( cipherEl ) {
		var pool = [ '█', '▓', '▒', '░', '●', '◆', '■', '◼', '▪', '◈', '✦' ];
		var texts = [ 'Formulardaten', 'Sichere Übertragung', 'Gesendet' ];
		var textIndex = 0;
		var charIndex = 0;
		...
		function tick() {
			if ( charIndex < chars.length ) {
				...
				cipherEl._bdfrmsTimer = setTimeout( tick, delay );
			} else {
				/* Alle Zeichen ersetzt – kurze Pause, dann nächster Klartext */
				cipherEl._bdfrmsTimer = setTimeout( function () {
	 ...
	 * @param {HTMLFormElement} form
	 */
	function bdfrmsShowSubmitOverlay( form ) {
		/* Button sperren */
		var wrapper = form.closest( '.bdfrms-form-wrapper' );
		...
		overlay.className = 'bdfrms-submit-overlay';
		overlay.setAttribute( 'role', 'alert' );
		overlay.setAttribute( 'aria-live', 'assertive' );

		var content = document.createElement( 'div' );
		content.className = 'bdfrms-submit-overlay__content';

		var cipher = document.createElement( 'div' );
		cipher.className = 'bdfrms-submit-overlay__cipher';
		cipher.setAttribute( 'aria-hidden', 'true' );

		var barTrack = document.createElement( 'div' );
		barTrack.className = 'bdfrms-submit-overlay__bar-track';

		var bar = document.createElement( 'div' );
		bar.className = 'bdfrms-submit-overlay__bar';
		barTrack.appendChild( bar );

		var message = document.createElement( 'p' );
		message.className = 'bdfrms-submit-overlay__message';
		message.textContent = 'Deine Angaben werden sicher übermittelt \u2026';
		...
		bdfrmsRunCipherAnimation( cipher );
	}

	function initForm( db, form ) {
		var key = form.getAttribute( 'data-bdfrms-key' );
		if ( ! key ) {
			return;
		}
		bdfrmsUpgradeWebKitDateTimeInputs( form );
		...
		var draftEnabledField = form.querySelector( 'input[name="bdfrms_draft_enabled"]' );
		var draftModeField = form.querySelector( 'input[name="bdfrms_draft_mode"]' );
		var draftTtlField = form.querySelector( 'input[name="bdfrms_draft_ttl_days"]' );
		...
		var resetButton = form.querySelector( '.bdfrms-draft-reset-button' );
			...
			resetButton.addEventListener( 'click', function ( e ) {
				...
				removeDraft( db, key )
					.then( function () {
						...
						form
							.querySelectorAll(
								'input[type="date"], input[type="time"], input[type="datetime-local"], input[data-bdfrms-wk-fallback="1"]'
							)
							.forEach( function ( el ) {
								el.value = '';
						...
						form.querySelectorAll( '.bdfrms-file-clear' ).forEach( function ( btn ) {
							btn.setAttribute( 'hidden', '' );
						} );
						initRangeValueDisplays( form );
						bdfrmsStripSubmitStateFromUrlIfPresent();
						bdfrmsDismissSubmitNoticeForForm( form );
		...
		form.querySelectorAll( 'input[type="file"]' ).forEach( function ( fileInput ) {
			var clearBtn = fileInput.closest( '.bdfrms-field' )
				? fileInput.closest( '.bdfrms-field' ).querySelector( '.bdfrms-file-clear' )
				: null;
			if ( ! clearBtn ) {
				return;
			}
		...
		form.addEventListener(
			'submit',
			function () {
				bdfrmsShowSubmitOverlay( form );
				try {
					var sk = form.getAttribute( 'data-bdfrms-key' );
					if ( sk && window.sessionStorage ) {
						window.sessionStorage.setItem(
							'bdfrms_submit_snapshot:' + sk,
	 ...
	 */
	function bdfrmsNormalizeKeyMap( raw ) {
		var out = {};
	...
	function bdfrmsReplaceTokenPlaceholders( s, values, labels ) {
	 ...
	 * @param {Record<string,string>} labels
	 */
	function bdfrmsApplyTokensToTextNodes( root, values, labels ) {
		...
		list.forEach( function ( textNode ) {
			var next = bdfrmsReplaceTokenPlaceholders( textNode.nodeValue, values, labels );
	 ...
	 * @return {void}
	 */
	function bdfrmsApplySubmitSnapshotTokens() {
		var sp = new URLSearchParams( window.location.search );
		if ( sp.get( 'bdfrms_status' ) !== 'success' ) {
			return;
		}
		var formId = sp.get( 'bdfrms_form' );
		if ( ! formId ) {
			return;
		}
		var roots = document.querySelectorAll( '.bdfrms-form-wrapper[data-bdfrms-await-tokens]' );
		if ( ! roots.length ) {
			return;
		}
		var formIdLc = String( formId ).toLowerCase();
		Array.prototype.forEach.call( roots, function ( wrap ) {
			var wrapFormId = wrap.getAttribute( 'data-bdfrms-form-id' );
			if ( String( wrapFormId || '' ).toLowerCase() !== formIdLc ) {
				return;
			}
			var labels = {};
			var lm = wrap.getAttribute( 'data-bdfrms-label-map' );
			if ( lm ) {
				try {
					var parsed = JSON.parse( lm );
					if ( parsed && typeof parsed === 'object' && ! Array.isArray( parsed ) ) {
						labels = bdfrmsNormalizeKeyMap( parsed );
					}
				} catch ( parseErr ) {
					labels = {};
				}
			}
			var key = wrap.getAttribute( 'data-bdfrms-key' );
			if ( ! key || ! window.sessionStorage ) {
				return;
			}
			var raw = window.sessionStorage.getItem( 'bdfrms_submit_snapshot:' + key );
			var values = {};
			if ( raw ) {
				try {
					var vobj = JSON.parse( raw );
					if ( vobj && typeof vobj === 'object' && ! Array.isArray( vobj ) ) {
						values = bdfrmsNormalizeKeyMap( vobj );
					}
				} catch ( jsonErr ) {
					values = {};
				}
			}
			bdfrmsApplyTokensToTextNodes( wrap, values, labels );
			try {
				window.sessionStorage.removeItem( 'bdfrms_submit_snapshot:' + key );
			} catch ( rmErr ) {
				/* ignorieren */
			}
		} );
	...
	function bdfrmsStripSubmitStateFromUrlIfPresent() {
		var params = new URLSearchParams( window.location.search );
		var keys = [ 'bdfrms_status', 'bdfrms_code', 'bdfrms_form', 'bdfrms_detail', 'bdfrms_draft_key' ];
		var changed = false;
	 ...
	 * @return {void}
	 */
	function bdfrmsDismissSubmitNoticeForForm( form ) {
		var wrap = form.closest( '.bdfrms-form-wrapper' );
		if ( ! wrap ) {
			return;
		}
		var note = wrap.querySelector( '.bdfrms-notice' );
		if ( note && note.parentNode ) {
			note.parentNode.removeChild( note );
		}
	}

	function bdfrmsInitFrontend() {
		/* Platzhalter zuerst: `bdfrms_status`/`bdfrms_form` stehen noch in der URL; kein Warten auf IndexedDB. */
		bdfrmsApplySubmitSnapshotTokens();

		var sp = new URLSearchParams( window.location.search );
		if ( sp.get( 'bdfrms_status' ) === 'success' && sp.get( 'bdfrms_draft_key' ) ) {
			var dk = sp.get( 'bdfrms_draft_key' );
		...
		var forms = document.querySelectorAll( 'form.bdfrms-form[data-bdfrms-key]' );
	...
	if ( document.readyState === 'loading' ) {
		document.addEventListener( 'DOMContentLoaded', bdfrmsInitFrontend );
	} else {
		bdfrmsInitFrontend();
	}
} )();
