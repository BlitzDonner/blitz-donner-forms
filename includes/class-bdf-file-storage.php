 ...
 *
 * Uploads landen im Uploads-Ordner unter `bdf-files/` mit Zufallsnamen und
 ...
 *
 * Erweiterungspunkt: Der Filter `bdf_store_file` erlaubt Add-ons, die
 ...
 */
class BDF_File_Storage {

	/**
	 * Unterordner im Uploads-Verzeichnis.
	 */
	const STORAGE_SUBDIR = 'bdf-files';
	...
	public static function store( array $file, array $context ) {
		 ...
		 */
		$handled = apply_filters( 'bdf_store_file', null, $file, $context );
		if ( null !== $handled ) {
			return $handled;
		}

		if ( ! self::ensure_storage_directory() ) {
			return new WP_Error( 'bdf_storage_unavailable', __( 'Ablageverzeichnis nicht beschreibbar.', 'blitz-donner-forms' ) );
		}
		...
		if ( ! $moved ) {
			return new WP_Error( 'bdf_store_failed', __( 'Datei konnte nicht abgelegt werden.', 'blitz-donner-forms' ) );
		}
		...
		if ( false === $inserted ) {
			...
			return new WP_Error( 'bdf_store_failed', __( 'Datei-Metadaten konnten nicht gespeichert werden.', 'blitz-donner-forms' ) );
		}
	...
	public static function handle_download() {
		...
		check_admin_referer( 'bdf_download_' . $file_id );
		...
		/**
		 * Datei-Auslieferung übernehmen (Gegenstück zu `bdf_store_file`).
		 *
		 ...
		 * @param array $row     Metadaten-Zeile aus {prefix}bdf_files.
		 */
		$handled = apply_filters( 'bdf_export_file', false, $row );
		if ( true === $handled ) {
			exit;
		}
