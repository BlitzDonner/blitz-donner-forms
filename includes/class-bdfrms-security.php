...
/**
 * Serverseitige Sicherheitsbausteine: Client-IP, HMAC-Token, Honeypot-
 * Feldnamen, Datei-Validierung, Sicherheits-Events.
 */
class BDFRMS_Security {

	const TOKEN_TTL_SECONDS = 3600;
	const TOKEN_MIN_AGE     = 2;
	const UPLOAD_SUBDIR     = 'bdfrms';
	...
	// Abschnitt: Security-Event-Logger.
	 ...
	 *
	 * Konfiguration:
	 *   add_filter( 'bdfrms_trusted_proxies', function () {
	...
	public static function get_client_ip() {
		 ...
		 */
		$trusted = apply_filters( 'bdfrms_trusted_proxies', array() );
		...
		$xff = isset( $_SERVER['HTTP_X_FORWARDED_FOR'] ) ? sanitize_text_field( wp_unslash( (string) $_SERVER['HTTP_X_FORWARDED_FOR'] ) ) : '';
		if ( '' === $xff ) {
			return $remote;
		}
	...
	public static function maybe_pseudonymize_ip( $ip ) {
		 ...
		 */
		if ( ! apply_filters( 'bdfrms_pseudonymize_ip', false ) ) {
			return $ip;
		}
	...
	// Abschnitt: Anti-Replay-HMAC-Token + Honeypot.
	...
	private static function token_key() {
		return wp_salt( 'auth' ) . '|bdfrms-anti-replay-v1';
	}
	 ...
	 * @param int    $post_id     Post-ID.
	 * @param string $form_id     Form-ID.
	 * @param string $instance_id Form-Instanz.
	 * @param string $hp_field    Honeypot-Feldname.
	...
	public static function honeypot_field_name( $post_id, $form_id, $instance_id ) {
		$hash = hash_hmac(
			'sha256',
			'bdfrms-honeypot-v1|' . (int) $post_id . '|' . $form_id . '|' . $instance_id,
			self::token_key()
		);
		return 'bdfrms_h_' . substr( $hash, 0, 12 );
	}

	// Abschnitt: Datei-Upload-Validation.
	...
	public static function default_allowed_mimes_map() {
		 ...
		 *   add_filter( 'bdfrms_allowed_mimes', function ( $map ) {
		 ...
		 */
		return apply_filters( 'bdfrms_allowed_mimes', $map );
	}
	...
	public static function blocked_extension_tokens() {
		 ...
		 */
		return apply_filters( 'bdfrms_blocked_extension_tokens', $blocked );
	}
	...
	public static function validate_uploaded_file( array $file_array, $accept = '' ) {
		...
		if ( '' === $original || '' === $tmp || ! is_uploaded_file( $tmp ) ) {
			...
			return new WP_Error( 'bdfrms_file_invalid', __( 'Ungültiger Datei-Upload.', 'blitz-donner-forms' ) );
		}
		...
		if ( preg_match( '/[\x00-\x1F\x7F]|\\\\/', $basename ) ) {
			...
			return new WP_Error( 'bdfrms_file_name', __( 'Ungültiger Dateiname.', 'blitz-donner-forms' ) );
		}
		...
		foreach ( $segments as $seg ) {
			if ( in_array( $seg, $blocked, true ) ) {
				...
				return new WP_Error( 'bdfrms_file_ext', __( 'Dieser Dateityp ist nicht erlaubt.', 'blitz-donner-forms' ) );
			}
		}

		$ext_index = strrpos( $lower, '.' );
		if ( false === $ext_index || strlen( $lower ) - 1 === $ext_index ) {
			self::log_event( 'file_reject_no_ext' );
			return new WP_Error( 'bdfrms_file_ext', __( 'Dateien ohne Endung sind nicht erlaubt.', 'blitz-donner-forms' ) );
		}
		...
		if ( '' === $detected ) {
			...
			return new WP_Error( 'bdfrms_file_mime', __( 'Dateityp konnte nicht verifiziert werden.', 'blitz-donner-forms' ) );
		}
		...
		if ( ! in_array( $detected_lower, $valid_mimes, true ) ) {
			...
			return new WP_Error( 'bdfrms_file_mime', __( 'Dateiinhalt passt nicht zur Endung.', 'blitz-donner-forms' ) );
		}
		...
		if ( '' !== $accept && ! self::accept_matches( $accept, $ext, $detected_lower ) ) {
			...
			return new WP_Error( 'bdfrms_file_accept', __( 'Dateityp ist für dieses Feld nicht erlaubt.', 'blitz-donner-forms' ) );
		}
	 ...
	 *
	 * @param string $base Absoluter Pfad (z. B. wp-content/uploads/bdfrms).
	 * @return void
	 */
	 ...
	 * komplett unter Plugin-Kontrolle hält: bdfrms_<random>.<ext>.
	 *
	...
	public static function unique_filename_callback( $ext ) {
		return static function ( $dir, $name, $ext_unused ) use ( $ext ) {
			...
			$candidate = 'bdfrms_' . $random . '.' . $ext;
			$try       = $candidate;
			$i         = 1;
			while ( file_exists( trailingslashit( $dir ) . $try ) ) {
				$try = 'bdfrms_' . $random . '_' . $i . '.' . $ext;
				++$i;
				if ( $i > 50 ) {
					break;
				}
	...
	// Abschnitt: DSGVO: Personal Data.

	/**
	 * Register personal data exporter.
	 *
	 * @param array<string,array<string,mixed>> $exporters Registrierte Exporter.
	 * @return array<string,array<string,mixed>>
	 */
	public static function register_personal_data_exporter( $exporters ) {
		$exporters['blitz-donner-forms'] = array(
			'exporter_friendly_name' => __( 'Blitz & Donner Formular', 'blitz-donner-forms' ),
	 ...
	 *
	 * @param array<string,array<string,mixed>> $erasers Registrierte Eraser.
	 * @return array<string,array<string,mixed>>
	 */
	public static function register_personal_data_eraser( $erasers ) {
		$erasers['blitz-donner-forms'] = array(
			'eraser_friendly_name' => __( 'Blitz & Donner Formular', 'blitz-donner-forms' ),
	...
	public static function export_personal_data( $email_address, $page = 1 ) {
		global $wpdb;
		$table = $wpdb->prefix . 'bdfrms_submissions';
		...
		if ( is_array( $rows ) ) {
			foreach ( $rows as $row ) {
				...
				$items = array(
					array(
						'name'  => __( 'Datum', 'blitz-donner-forms' ),
						'value' => (string) $row->created_at,
					),
					array(
						'name'  => __( 'Formular-ID', 'blitz-donner-forms' ),
						'value' => (string) $row->form_id,
					),
					array(
						'name'  => __( 'Formularname', 'blitz-donner-forms' ),
						'value' => isset( $row->form_title ) ? (string) $row->form_title : '',
					),
					array(
						'name'  => __( 'Beitrag', 'blitz-donner-forms' ),
						'value' => (string) $row->post_id,
					),
					array(
						'name'  => __( 'IP-Adresse', 'blitz-donner-forms' ),
						'value' => (string) $row->ip_address,
					),
					array(
						'name'  => __( 'User-Agent', 'blitz-donner-forms' ),
				...
				$data[] = array(
					'group_id'    => 'bdfrms-submissions',
					'group_label' => __( 'Formular-Einsendungen', 'blitz-donner-forms' ),
	 ...
	 *
	 * @param string $email_address E-Mail.
	 * @param int    $page          Seiten-Nummer des Erasers.
