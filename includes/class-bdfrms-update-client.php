 ...
 *   new BDFRMS_Update_Client( array(
 *       'plugin_file' => __FILE__,                 // Hauptdatei des Plugins
 *       'slug'        => 'blitz-donner-forms',  // Slug auf dem Server
 *       'server_url'  => 'https://plugins.blitzdonner.ch',
 *       'version'     => '0.7.0',                   // aktuelle Version
 *       'option_key'  => 'bdfrms_update_token',        // Einstellungs-Option (DB)
 *       'const_key'   => 'BDFRMS_UPDATE_TOKEN',         // wp-config-Konstante (Vorrang)
 *   ) );
 *
...
if ( ! class_exists( 'BDFRMS_Update_Client' ) ) :

class BDFRMS_Update_Client {
	...
	/** @var string plugin_basename (z.B. blitz-donner-forms/blitz-donner-forms.php). */
	private $basename;
	...
	/**
	 * Public-Modus für Gratis-Plugins: Anfragen laufen ohne Lizenz-Token
	 * (der Server-Eintrag muss öffentlich ausliefern). Signatur- und
	 * Prüfsummen-Checks bleiben unverändert Pflicht.
	 *
	 * @var bool
	 */
	...
	public function __construct( $args ) {
		...
		$this->is_public   = ! empty( $args['public'] );
	...
	private function fetch_remote() {
		...
		// SICHERHEIT: Token (falls vorhanden) im Header, nie in der URL.
	...
	public function maybe_license_notice() {
		// Gratis-Plugin ohne Lizenz: keine Lizenz-Hinweise.
		if ( $this->is_public ) {
			return;
		}
		if ( ! current_user_can( 'update_plugins' ) ) {
